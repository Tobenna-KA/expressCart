...
const calculateTotalCart = async (req, cart, currency) => {
    const config = getConfig();
    const db = req.app.db;
    ...
    Object.keys(cart).forEach((item) => {
        totalCartAmount = totalCartAmount + cart[item].totalItemPrice;
        totalCartProducts = totalCartProducts + cart[item].quantity;
    });

    // Update the total amount not including shipping/discounts
    let totalCartNetAmount = totalCartAmount;

    // Calculate shipping using the loaded module
    let totalCartShipping = config.modules.loaded.shipping.calculateShippingCost(
        totalCartNetAmount,
        currency,
        config,
        req
    ...
    console.log(totalCartShipping, '')
    ...
    if(config.modules.loaded.discount){
        ...
        const discount = await db.discounts.findOne({ code: req.session.discountCode });
        if(discount){
            totalCartDiscount = config.modules.loaded.discount.calculateDiscount(
                discount,
                req,
                true
            );
    ...
    // Calculate our total amount removing discount and adding shipping
    return (totalCartNetAmount - totalCartDiscount) + totalCartShipping;
};
