...
// parse cart for currency change
const parseCart = (cart, currency) => {
  console.log(cart, currency, 'HEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE');
  if (currency === 'productPrice') currency = 'productPriceKES';
  if (!cart) return {};
  // get cart keys
  const keys = Object.keys(cart);

  for (const key of keys) {
    // update each item based on the quantity and price in respective currency
    cart[key].totalItemPrice = cart[key].quantity * cart[key][currency];
  }
  return cart;
};

// function to get which currency field to look at
const getCurrencyField = (req, variant = false) => {
  if (req.cookies && req.cookies.currency) {
    // get currency from cookies
    req.session.currency = req.cookies.currency; // make sure they are both on the same page
    console.log(req.session.currency, 'CURRENCY');
    if (req.cookies.currency && req.cookies.currency !== 'productPrice') {
      if (req.cookies.currency === 'KES') return 'productPrice';
      else return variant ? 'price' : req.cookies.currency;
    } else {
...
const performCalculatePriceCartInDollars = (cart) => {};
...
module.exports = {
  ...
  parseCart,
  dateDiff,
  performCalculatePriceCartInDollars,
};
