...
// The homepage of the site
router.post('/paypal_order', (req, res, next) => {
    const db = req.app.db;
    const config = req.app.config;
    ...
    db.orders.insertOne(orderDoc, (err, newDoc) => {
        if (err) {
            ...
            res.status(500).send({
                failed: true,
                message: 'Order creation failed, please contact us.',
                redirectUrl: '/checkout/payment'
            })
        }
        const order = newDoc.ops[0]
        ...
        try {
            ...
            paymentStatus = 'Paid';
            paymentMessage = 'Your payment was successfully completed';
            paymentDetails = `<p><strong>Order ID: </strong>${newDoc.insertedId}</p><p><strong>Transaction ID: </strong>${payment.id}</p>`;
            ...
            indexOrders(req.app)
                .then(() => {
                    ...
                    res.status(200).send({
                        failed: false,
                        message: 'Success',
                        redirectUrl: `/payment/${order._id}`
                    });
                });
        } catch (e) {
            res.send(500).send({failed: true,  message: 'Failed to save order, please contact us'})
        }
    });
});
